...
* IoC（反向控制）/DI（依赖注入）在常规编程思想中，如果A对象中依赖B对象，那么我们会在A对象中主动去控制B对象的实例化，那么在抽象过程中为了消除创建对象减少依赖耦合度，我们会将B对象放入一个容器中去实例化，A对象中依赖注入B即可，依赖注入都主要方式有：属性注入、构造方法注入等。DI是IoC思想的一种应用。
